...
const Chatbot6 : React.FC = () => {
  ...
  const [isBotThinking, setIsBotThinking] = useState(false);  // New state to track bot's thinking status
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    setMessage('');
    setIsBotThinking(true); // Set thinking state to true
    ...
    // Simulate a delay before displaying the bot response
    setTimeout(() => {
      setChatHistory(prev => {
      ...
      // Update the chat_id once the bot response is received
      ...
      setChatHistory(prev => {
        const updatedHistory = [...prev];
        updatedHistory[updatedHistory.length - 1].chat_id = data.chat_id; // Assign chat_id
        return updatedHistory;
      });

      setIsBotThinking(false); // Set thinking state to false after response
    }, 3000);  // Add a 2-second delay (you can adjust the delay time as needed)
  };
    ...
    if (popupVisible.index !== null) {
      ...
      if (isLike) {
        if (currentChat.disliked) {
          ...
          currentChat.reason = null; // Clear the reason
        }
  ...
  const handleDislikeReasonSubmit = (index: number) => {
    if (dislikeReason && index !== null) {
      ...
      setDislikeReason(null); // Clear input
    }
    setPopupVisible({ visible: false, x: 0, y: 0, index: null });
  };
    ...
    <div className="p-1 bg-gray-100 flex items-center justify-center mx-0 ">
      <div className="bg-white shadow-md rounded-lg p-4 w-full md:w-1/2 lg:w-full">
        <div className="flex items-center justify-between bg-black text-white h-20 mx-0 md:flex-row">
          ...
          <h2 className="text-2xl font-bold text-blue-400 md:text-lg sm:text-xs lg:text-2xl">HR AI CHAT BOARD</h2>
        </div>
        <div className="overflow-y-auto h-[600px] mb-4">
          ...
          {/* Show loading indicator when bot is thinking */}
          {isBotThinking && (
